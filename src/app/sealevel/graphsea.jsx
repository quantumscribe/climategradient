...
import jsonData from './sealevel.json';

function Graphsealevel() {
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    function drawChart(options) {
      ...
      const yScale = d3.scaleLinear()
        .domain([-25, d3.max(data, d => d.sealevel)])
        .nice()
        .range([height, 0]);
      ...
      svg.on('mousemove', function(event) {
        ...
        const sealevelAtMouseY = yScale.invert(mouseY);
        ...
        if (d) {
          ...
          yLine.attr('y1', yScale(d.sealevel))
            .attr('y2', yScale(d.sealevel))
            .style('opacity', 1);
        
          const tooltipWidth = tooltipRef.current.offsetWidth;
          const tooltipHeight = tooltipRef.current.offsetHeight;
        
          tooltip.html(`<strong style="color: black;">Date:</strong> <span style="color: black;">${d.date.toLocaleString('default', { month: 'short' })} ${d.date.getFullYear()}</span><br><strong style="color: black;">Sea level rise (mm):</strong> <span style="color: black;">${d.sealevel}</span>`)
            .style('left', `${xScale(d.date) + margin.left}px`) // Position to the right of the data point
            .style('top', `${yScale(d.sealevel) + margin.top + 50}px`) // Position slightly below the data point with more space
            .style('opacity', 1);
        } else {
          chartGroup.selectAll('.dot').remove();
  ...
  return (
    <div>
      <div style={{ position: 'relative' }}>
        <h2 className="text-xl font-bold text-white-800 px-4 py-2 border-b border-white-200">Yearly Rise in Global SeaLevel</h2>
