...
import jsonData from './methane.json';

function Graphmethane() {
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    function drawChart(options) {
      ...
      const yScale = d3.scaleLinear()
        .domain([1600, d3.max(data, d => d.methanelevel)])
        .nice()
        .range([height, 0]);
      ...
      svg.on('mousemove', function(event) {
        ...
        const methanelevelAtMouseY = yScale.invert(mouseY);
        ...
        if (d) {
          ...
          yLine.attr('y1', yScale(d.methanelevel))
            .attr('y2', yScale(d.methanelevel))
            .style('opacity', 1);
        
          const tooltipWidth = tooltipRef.current.offsetWidth;
          const tooltipHeight = tooltipRef.current.offsetHeight;
        
          tooltip.html(`<strong style="color: black;">Date:</strong> <span style="color: black;">${d.date.toLocaleString('default', { month: 'short' })} ${d.date.getFullYear()}</span><br><strong style="color: black;">Methane level rise (ppd):</strong> <span style="color: black;">${d.methanelevel}</span>`)
            .style('left', `${xScale(d.date) + margin.left}px`) // Position to the right of the data point
            .style('top', `${yScale(d.methanelevel) + margin.top + 50}px`) // Position slightly below the data point with more space
            .style('opacity', 1);
        } else {
          chartGroup.selectAll('.dot').remove();
      ...
      const xAxis = d3.axisBottom(xScale).ticks(d3.timeYear.every(4)).tickFormat(d3.timeFormat('%Y')).tickSizeOuter(0).tickPadding(10);
  ...
  return (
    <div>
      <div style={{ position: 'relative' }}>
        <h2 className="text-xl font-bold text-white-800 px-4 py-2 border-b border-white-200">Yearly Rise in Global Methane</h2>
